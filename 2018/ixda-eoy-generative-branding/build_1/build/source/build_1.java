...
public void noiseField(PVector[][] nField) {
  for(int j = 0; j < nField.length; j++) {
    for(int i = 0; i < nField[0].length; i++) {
      PVector pv = PVector.fromAngle(1.7f * TWO_PI * noise(j*noiseStep, i*noiseStep));
      // pv.x += i*.01;
      // pv.y += j*.01;
      // nField[j][i] = pv;

      float tX = ((PApplet.parseFloat(i) + .5f) / col);
      float tY = ((PApplet.parseFloat(j) + .5f) / row);
...
public void keyPressed() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#333333", "--stop-color=#cccccc", "build_1" };
    if (passedArgs != null) {
